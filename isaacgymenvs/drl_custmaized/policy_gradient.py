...
class PGBuffer(BaseBuffer):
    ...
    def finish_path(self, last_val=0):
        ...
        return_temp = discount_cumsum(reward_temp, self.gamma)[:-1]
        return_temp -= np.mean(return_temp)
        return_temp /= np.std(return_temp + 1e-8)
        # the next line computes rewards-to-go, to be targets for the value function
        self.ret_buf[path_slice] = return_temp
